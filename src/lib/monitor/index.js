...
export const userActivityTracking = (user_id, op_code, activity_status) => {
  if (user_id) {
    ...
    return monitor(logQueries.createActivityLogs, [
      user_id,
      operation,
      activity_status
...
export const adminActivityTracking = (user_id, op_code, activity_status) => {
  if (user_id) {
    const operation = operations[op_code];
    return monitor(logQueries.createAdminActivityLogs, [
      user_id,
      operation,
      activity_status
    ], 'adminActivityTracking', operation, user_id);
  }
};
