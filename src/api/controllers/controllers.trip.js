...
export const fetchAllBookings = async (req, res) => {
  try {
    ...
    const [ bookings, [ bookingsCount ] ] = await TripServices.fetchAllBookings(payload);
    logger.info(
    ...
    const data = {
      ...
      total_pages: Helpers.calculatePages(Number(bookingsCount.count), Number(req.query.per_page) || 10),
      bookings
    };
    return ApiResponse.success(
      res,
...
export const fetchAllUserBookings = async (req, res) => {
  try {
    ...
    const [ userBookings, [ userBookingsCount ] ] = await TripServices.fetchAllUserBookings(payload);
    logger.info(
    ...
    const data = {
      ...
      total_pages: Helpers.calculatePages(Number(userBookingsCount.count), Number(req.query.per_page) || 10),
      userBookings
    };
    return ApiResponse.success(
      res,
...
export const filterTrips = async (req, res) => {
  try {
    ...
    logger.info(`${enums.CURRENT_TIME_STAMP}, ${userId} Info: successfully filtered trips by origin or destination filterTripsByOriginOrDestination.trips.controllers.roles.js`);
    const data = {
      page: parseFloat(req.query.page) || 1,
      total_count: Number(tripsCount.count),
      total_pages: Helpers.calculatePages(Number(tripsCount.count), Number(req.query.per_page) || 10),
      trips
    };
    return ApiResponse.success(res, enums.FILTER_TRIPS, 200, data );
  } catch (error) {
    error.label = enums.FILTER_TRIPS_CONTROLLER;
    logger.error(`filtering trips failed::${enums.FILTER_TRIPS_CONTROLLER}::::${error.message}`);
    return error;
  }
};
