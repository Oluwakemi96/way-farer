  ...
  it("Should create user one successfully", (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        process.env.WAYFARER_USER_ONE_VERIFICATION_TOKEN =
          res.body.data.email_token;
        done();
      });
  });
  ...
  it("Should create user two successfully", (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        process.env.WAYFARER_USER_TWO_VERIFICATION_TOKEN =
          res.body.data.email_token;
        done();
      });
  });
...
describe("Login", () => {
  it("Should regenerate user one email token successfully", (done) => {
    chai
      .request(app)
      .put(`/api/v1/auth/regenerate-email-token`)
      .send({
        email: userOneProfile.email,
      })
      .end((err, res) => {
        ...
        expect(res.body).to.have.property("status");
        expect(res.body).to.have.property("data");
        expect(res.body.message).to.equal(enums.REGENERATE_EMAIL_TOKEN);
        expect(res.body.status).to.equal(enums.SUCCESS_STATUS);
        process.env.WAYFARER_USER_ONE_VERIFICATION_TOKEN = res.body.data;
        done();
      });
  });

  it("Should not regenerate user one email token if email is not sent", (done) => {
    chai
      .request(app)
      .put(`/api/v1/auth/regenerate-email-token`)
      .send({})
  ...
  it("Should verify user one email successfully", (done) => {
    chai
      .request(app)
      .put(
        `/api/v1/auth/verify-email/${process.env.WAYFARER_USER_ONE_VERIFICATION_TOKEN}`
      )
      .end((err, res) => {
        expect(res.statusCode).to.equal(enums.HTTP_OK);
        expect(res.body).to.have.property("message");
  ...
  it("Should verify user two email successfully", (done) => {
    chai
      .request(app)
      .put(
        `/api/v1/auth/verify-email/${process.env.WAYFARER_USER_TWO_VERIFICATION_TOKEN}`
      )
      .end((err, res) => {
        expect(res.statusCode).to.equal(enums.HTTP_OK);
        expect(res.body).to.have.property("message");
